...
read -rp "Do you want to update Metasploit? [y/N] " answer
if [[ ! "$answer" =~ ^[Yy]$ ]]; then
...
if ! git clone https://github.com/rapid7/metasploit-framework.git --depth=1 "${PREFIX}/opt/metasploit-framework"; then
    stop_spinner
    echo -e "\e[31mClone failed.\e[0m"
    exit 1
fi
...
wget -qO "${PREFIX}/opt/metasploit-framework/msfupdate" \
     "https://raw.githubusercontent.com/katilmamioffical/metasploit-in-termux/main/msfupdate" \
  && chmod +x "${PREFIX}/opt/metasploit-framework/msfupdate" \
  && ln -sf "${PREFIX}/opt/metasploit-framework/msfupdate" "${PREFIX}/bin/msfupdate"

wget -qO "${PREFIX}/opt/metasploit-framework/msfupdate.sh" \
     "https://raw.githubusercontent.com/katilmamioffical/metasploit-in-termux/main/msfupdate.sh" \
